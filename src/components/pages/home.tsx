...
export function HomePage() {
  ...
  return <div className="min-h-screen bg-ploy-background-primary font-body text-ploy-text-primary selection:bg-ploy-accent-primary selection:text-ploy-text-on-accent-primary">
    ...
    <main id="top">
      <section className="hero mx-auto grid max-w-[1440px] gap-8 px-5 pb-14 pt-10 md:px-10 lg:grid-cols-[0.9fr_1.1fr] lg:items-end lg:pb-24 lg:pt-16">
        <div className="hero__copy max-w-2xl pb-2">
          <p className="mb-8 max-w-md text-sm font-semibold uppercase tracking-[0.14em] text-ploy-accent-primary">{isDe ? "Interior Design für möblierte Vermietung · Wien" : "Interior design for furnished rentals · Vienna"}</p>
      ...
      <section id="neu-einrichten" className="scroll-mt-24 px-5 py-20 md:px-10 lg:py-32"><div className="mx-auto grid max-w-[1200px] gap-14 lg:grid-cols-[0.85fr_1.15fr]">
        ...
        <div className="grid gap-px bg-ploy-border-primary sm:grid-cols-2">{(isDe ? [["6–8", "Wochenenden für Recherche, Einkauf und Aufbau"], ["€3–6k", "gehen häufig durch falsche Maße, Haltbarkeit oder Retouren verloren"], ["12", "Wochen Lieferzeit können den Übergabetermin kippen"], ["⅓", "unteres Drittel des Preisbands, wenn Inseratsfotos flach wirken"]] : [["6–8", "weekends spent researching, shopping and assembling"], ["€3–6k", "often lost to wrong sizes, poor durability or returns"], ["12", "week lead times that can derail the handover"], ["⅓", "bottom third of the price band when listing photos feel flat"]]).map(([n, d]) => <div key={d} className="bg-ploy-background-secondary p-7"><div className="font-heading text-5xl">{n}</div><p className="mt-4 text-sm leading-relaxed text-ploy-text-secondary">{d}</p></div>)}</div>
      </div></section>
      ...
      <section className="border-y border-ploy-border-primary bg-ploy-background-secondary px-5 py-20 md:px-10 lg:py-28"><div className="mx-auto max-w-[1200px]"><div className="grid gap-10 lg:grid-cols-[1.1fr_0.9fr] lg:items-end"><img src={PROOF_IMAGE} alt={isDe ? "Jubili Suite mit blauer Akzentwand" : "Jubili suite with a blue feature wall"} className="h-[520px] w-full object-cover" /><div><p className="text-sm font-semibold uppercase tracking-widest text-ploy-accent-primary">{isDe ? "Ausgewähltes Einrichtungsbeispiel" : "Selected furnishing example"}</p><h2 className="mt-5 text-balance font-heading text-4xl md:text-6xl">{t.proofTitle}</h2><blockquote className="mt-8 border-l-2 border-ploy-accent-primary pl-5 font-heading text-2xl leading-snug">{t.proofQuote}</blockquote></div></div>
        ...
        <div className="mt-8 grid gap-3 md:grid-cols-12">{GALLERY_IMAGES.map((image, index) => <figure key={image.src} className={image.className}><img src={image.src} alt={image.alt} loading="lazy" className={`w-full object-cover ${index < 2 ? "h-[420px]" : "h-[360px]"}`} /></figure>)}</div>
      </div></section>

      <section className="px-5 py-20 md:px-10 lg:py-28"><div className="mx-auto max-w-[1200px]"><div className="grid gap-12 lg:grid-cols-2"><h2 className="text-balance font-heading text-4xl md:text-6xl">{t.whyTitle}</h2><div className="space-y-0">{(isDe ? [["Grundriss und Kosten", "Ich lese den Bestand, erkenne, welche Wand sich bewegt — und was diese Entscheidung kostet."], ["Für intensive Nutzung spezifiziert", "15 Jahre Architektur, auch im Gesundheits- und institutionellen Bereich: robust, verständlich und trotzdem ruhig."], ["Selbst im Betrieb", "Ich kenne Übergaben, Turnovers und Gästefragen nicht nur aus Moodboards, sondern aus dem eigenen Objekt."]] : [["Plans and costs", "I read the existing structure, know which wall can move — and what that decision costs."], ["Specified for heavy use", "Fifteen years in architecture, including healthcare and institutions: durable, legible and still calm."], ["An operator too", "I know handovers, turnovers and guest questions from running a property, not only from mood boards."]]).map(([h, p]) => <div key={h} className="border-t border-ploy-border-primary py-6"><h3 className="font-heading text-2xl">{h}</h3><p className="mt-2 leading-relaxed text-ploy-text-secondary">{p}</p></div>)}</div></div>
    ...
    <footer className="border-t border-ploy-border-primary px-5 py-8 md:px-10"><div className="mx-auto flex max-w-[1200px] flex-col gap-6 text-sm md:flex-row md:items-center md:justify-between"><div className="flex items-center gap-3"><strong className="font-heading text-xl">Jubili</strong><span className="text-ploy-text-secondary">Vienna</span></div><nav className="flex flex-wrap gap-5"><a href="#" aria-label="Instagram">Instagram</a><a href="#" aria-label="LinkedIn">LinkedIn</a><a href="mailto:office@jubili.co">E-Mail</a><a href="/impressum">Impressum</a><a href="/datenschutz">Datenschutz</a></nav></div></footer>
  </div>;
}
