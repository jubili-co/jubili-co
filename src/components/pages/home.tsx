...
const PROOF_IMAGE = "/images/proof.webp";
const GALLERY_IMAGES = [
  { src: "/images/gallery-living.webp", alt: "Jubili living room opening onto a leafy balcony", className: "md:col-span-7" },
  { src: "/images/gallery-balcony.webp", alt: "Furnished Jubili balcony with dining table and string lights", className: "md:col-span-5" },
  { src: "/images/gallery-attic.webp", alt: "Finished Jubili attic room with workspace", className: "md:col-span-5" },
  { src: "/images/gallery-dining.webp", alt: "Jubili dining room with green gallery wall", className: "md:col-span-7" },
];
    ...
    <main id="top">
      <section className="hero mx-auto grid max-w-[1440px] gap-8 px-5 pb-14 pt-10 md:px-10 lg:grid-cols-[0.9fr_1.1fr] lg:items-end lg:pb-24 lg:pt-16">
        <div className="hero__copy max-w-2xl pb-2">
          <p className="mb-8 max-w-md text-sm font-semibold uppercase tracking-[0.14em] text-brand-accent-primary">{isDe ? "Interior Design für möblierte Vermietung · Wien" : "Design for furnished rentals · Vienna"}</p>
          ...
          <p className="mt-7 max-w-xl text-lg leading-relaxed text-brand-text-secondary md:text-xl">{t.heroSub}</p>
          <div className="mt-8"><Cta label="hero_call">{t.call}</Cta><p className="mt-3 text-xs leading-relaxed text-brand-text-secondary">{t.heroMeta}</p></div>
        </div>
        <figure><img src={HERO_IMAGE} alt="Jubili furnished living room at Tegelweg in Vienna" className="hero__image h-[58vh] min-h-[480px] w-full object-cover object-center lg:h-[76vh]" /><figcaption className="mt-3 text-sm text-brand-text-secondary">Unit at Tegelweg, Vienna / Full package.</figcaption></figure>
      </section>
      ...
      <section id="neu-einrichten" className="scroll-mt-24 px-5 py-20 md:px-10 lg:py-32"><div className="mx-auto grid max-w-[1200px] gap-14 lg:grid-cols-[0.85fr_1.15fr]">
        <div><h2 className="text-balance font-heading text-4xl md:text-6xl">{t.launcherTitle}</h2><p className="mt-5 text-lg leading-relaxed text-brand-text-secondary">{t.launcherIntro}</p></div>
        <div className="grid gap-px bg-brand-border-primary sm:grid-cols-2">{(isDe ? [["6–8", "Wochenenden für Recherche, Einkauf und Aufbau"], ["€3–6k", "gehen häufig durch falsche Maße, Haltbarkeit oder Retouren verloren"], ["12", "Wochen Lieferzeit können den Übergabetermin kippen"], ["1/3", "unteres Drittel des Preisbands, wenn Inseratsfotos flach wirken"]] : [["6–8", "weekends spent researching, shopping and assembling"], ["€3–6k", "often lost to wrong sizes, poor durability or returns"], ["12", "week lead times that can derail the handover"], ["1/3", "bottom third of the price band when listing photos feel flat"]]).map(([n, d]) => <div key={d} className="bg-brand-background-secondary p-7"><div className={`font-heading text-5xl ${n === "1/3" ? "font-normal" : ""}`}>{n}</div><p className="mt-4 text-sm leading-relaxed text-brand-text-secondary">{d}</p></div>)}</div>
      </div></section>

      <section className="border-y border-brand-border-primary bg-brand-background-secondary px-5 py-20 md:px-10 lg:py-28"><div className="mx-auto max-w-[1200px]"><div className="mb-10 flex flex-col justify-between gap-5 md:flex-row md:items-end"><h2 className="max-w-2xl text-balance font-heading text-4xl md:text-6xl">{isDe ? "Drei klare Wege zur fertigen Wohnung." : "Three clear routes to a rental-ready apartment."}</h2><p className="max-w-md text-sm leading-relaxed text-brand-text-secondary">{isDe ? "Der erste Schritt bleibt überschaubar. Beim Vermietungs-Check wird das Honorar bei einem Folgeauftrag vollständig angerechnet." : "The first step stays contained. The rental viability check is fully credited if you continue with a full project."}</p></div>
        <div className="grid gap-px bg-brand-border-primary lg:grid-cols-3">{packages.map((pkg) => <article key={pkg.name} className="flex min-h-[390px] flex-col bg-brand-background-primary p-7 md:p-9"><p className="text-sm font-semibold text-brand-accent-primary">{pkg.time}</p><h3 className="mt-6 font-heading text-3xl">{pkg.name}</h3><p className="mt-3 font-heading text-5xl">{pkg.price}</p><p className="mt-6 flex-1 leading-relaxed text-brand-text-secondary">{pkg.text}</p><Cta label={pkg.event} className="mt-8 w-full">{lang === "de" ? "Gespräch buchen" : "Book a call"}</Cta></article>)}</div>
      </div></section>
      ...
      <section id="umstellen" className="scroll-mt-24 px-5 py-20 md:px-10 lg:py-32"><div className="mx-auto grid max-w-[1200px] gap-12 lg:grid-cols-2">
        <div><h2 className="text-balance font-heading text-4xl md:text-6xl">{t.converterTitle}</h2><p className="mt-6 text-lg leading-relaxed text-brand-text-secondary">{t.converterIntro}</p><p className="mt-4 border-l-2 border-brand-accent-primary pl-4 text-sm text-brand-text-secondary">{t.disclaimer}</p></div>
        <div className="border-t border-brand-border-primary pt-8 lg:border-l lg:border-t-0 lg:pl-12 lg:pt-0"><p className="font-heading text-3xl leading-tight">{t.opportunity}</p><div className="mt-9"><Cta label="pkg_check">{isDe ? "Vermietungs-Check · €490" : "Rental check · €490"}</Cta></div></div>
      </div></section>

      <section className="border-y border-brand-border-primary bg-brand-background-secondary px-5 py-20 md:px-10"><div className="mx-auto max-w-[1200px]"><h2 className="text-balance font-heading text-4xl md:text-6xl">{t.rolesTitle}</h2><p className="mt-4 max-w-2xl text-brand-text-secondary">{t.rolesSub}</p>
        <div className="mt-10 overflow-x-auto"><table className="w-full min-w-[680px] border-collapse text-left"><thead><tr className="border-b border-brand-border-primary"><th className="py-4 pr-4"></th><th className="py-4 pr-4 font-heading text-2xl">{isDe ? "Ich übernehme" : "I handle"}</th><th className="py-4 font-heading text-2xl">{isDe ? "Sie übernehmen" : "You handle"}</th></tr></thead><tbody>{(isDe ? [["Planung", "Aufmaß, Konzept, Layout, Spezifikation", "Zugang zur Wohnung, vorhandene Pläne"], ["Bestellung", "Sourcing, Warenkorb, Lieferkoordination", "Freigabe und Bezahlung der Bestellungen"], ["Logistik", "Lieferfenster, Abstimmung mit Lieferanten", "Gebäudezugang, Liftmaße, Hausverwaltung"], ["Umsetzung", "Aufbaukoordination, Styling, letzte Kontrolle", "Entsorgung nur nach Vereinbarung"], ["Vermarktung", "Fotografie-Briefing und Set-up", "Fotograf oder Zusatzbeauftragung"]] : [["Planning", "Survey, concept, layout, specification", "Property access and existing plans"], ["Ordering", "Sourcing, baskets and delivery coordination", "Approval and payment of orders"], ["Logistics", "Delivery windows and supplier coordination", "Building access, lift dimensions and management"], ["Installation", "Assembly coordination, styling and final check", "Disposal only by agreement"], ["Marketing", "Photography brief and set-up", "Photographer or additional commission"]]).map((row) => <tr key={row[0]} className="border-b border-brand-border-primary"><th className="py-5 pr-4 text-sm font-semibold">{row[0]}</th><td className="py-5 pr-4 text-brand-text-secondary">{row[1]}</td><td className="py-5 text-brand-text-secondary">{row[2]}</td></tr>)}</tbody></table></div>
      </div></section>

      <section className="px-5 py-20 md:px-10 lg:py-28"><div className="mx-auto max-w-[1200px]"><h2 className="text-balance font-heading text-4xl md:text-6xl">{t.timelineTitle}</h2><div className="mt-12 grid gap-px bg-brand-border-primary md:grid-cols-4">{(isDe ? [["Woche 1", "Gespräch und Aufmaß"], ["Woche 1–2", "Konzept"], ["Woche 2–4", "Sourcing und Bestellung"], ["Woche 4–6", "Lieferung und Styling"]] : [["Week 1", "Call and survey"], ["Weeks 1–2", "Concept"], ["Weeks 2–4", "Sourcing and ordering"], ["Weeks 4–6", "Delivery and styling"]]).map(([week, step]) => <div key={step} className="bg-brand-background-primary p-6"><p className="text-xs font-bold uppercase tracking-widest text-brand-accent-primary">{week}</p><p className="mt-8 font-heading text-3xl">{step}</p></div>)}</div></div></section>

      <section className="border-y border-brand-border-primary bg-brand-background-secondary px-5 py-20 md:px-10 lg:py-28"><div className="mx-auto max-w-[1200px]"><div className="grid gap-10 lg:grid-cols-[1.1fr_0.9fr] lg:items-end"><figure><img src={PROOF_IMAGE} alt={isDe ? "Jubili Schlafzimmer mit blauer Akzentwand in der Brabbeegasse" : "Jubili bedroom with a blue feature wall at Brabbeegasse"} className="aspect-[4/3] w-full object-cover object-center lg:aspect-auto lg:h-[520px]" /><figcaption className="mt-3 text-sm text-brand-text-secondary">Unit at Brabbeegasse, Vienna / Full package.</figcaption></figure><div><p className="text-sm font-semibold uppercase tracking-widest text-brand-accent-primary">{isDe ? "Ausgewähltes Einrichtungsbeispiel" : "Selected furnishing example"}</p><h2 className="mt-5 text-balance font-heading text-4xl md:text-6xl">{t.proofTitle}</h2><blockquote className="mt-8 border-l-2 border-brand-accent-primary pl-5 font-heading text-2xl leading-snug">{t.proofQuote}</blockquote></div></div>
        <div className="mt-14 grid gap-px bg-brand-border-primary md:grid-cols-3">{(isDe ? [["4,98/5", "Gästebewertung"], ["97,6%", "Fünf-Sterne-Aufenthalte"], ["€125k+", "Gästeumsatz in 5 Jahren"]] : [["4.98/5", "guest rating"], ["97.6%", "five-star stays"], ["€125k+", "guest revenue over 5 years"]]).map(([n, d]) => <div key={d} className="bg-brand-background-primary p-7"><div className="font-heading text-5xl">{n}</div><p className="mt-2 text-sm text-brand-text-secondary">{d}</p></div>)}</div><p className="mt-4 text-xs text-brand-text-secondary">{isDe ? "Eigenes Pilotobjekt, Wien · Jubili · fünf Jahre Betrieb · Wachstum von zwei auf vier Zimmer." : "Own pilot property, Vienna · Jubili · five years of operation · grown from two rooms to four."}</p>
        <div className="mt-20 flex flex-col justify-between gap-5 border-t border-brand-border-primary pt-10 md:flex-row md:items-end"><h3 className="max-w-2xl text-balance font-heading text-4xl md:text-5xl">{isDe ? "Ausgewählte Räume. Eine klare Betriebslogik." : "Selected rooms. One clear operating logic."}</h3><p className="max-w-md text-sm leading-relaxed text-brand-text-secondary">{isDe ? "Keine Kulissen: echte Räume aus dem eigenen Jubili Objekt, geplant für Gäste, Reinigung, Wartung und Wiedervermietung." : "Not set dressing: real rooms from Jubili’s own property, planned around guests, cleaning, maintenance and repeat stays."}</p></div>
        <div className="mt-8 grid gap-3 md:grid-cols-12">{GALLERY_IMAGES.map((image, index) => <figure key={image.src} className={image.className}><img src={image.src} alt={image.alt} loading="lazy" className={`w-full object-cover ${index < 2 ? "h-[420px]" : "h-[360px]"}`} /></figure>)}</div><p className="mt-3 text-sm text-brand-text-secondary">Unit at Tegelweg, Vienna / Full package.</p>
      </div></section>

      <section className="px-5 py-20 md:px-10 lg:py-28"><div className="mx-auto max-w-[1200px]"><div className="grid gap-12 lg:grid-cols-2"><h2 className="text-balance font-heading text-4xl md:text-6xl">{t.whyTitle}</h2><div className="space-y-0">{(isDe ? [["Grundriss und Kosten", "Ich lese den Bestand, erkenne, welche Wand sich bewegt, und was diese Entscheidung kostet."], ["Für intensive Nutzung spezifiziert", "15 Jahre Architektur, auch im Gesundheits- und institutionellen Bereich: robust, verständlich und trotzdem ruhig."], ["Selbst im Betrieb", "Ich kenne Übergaben, Turnovers und Gästefragen nicht nur aus Moodboards, sondern aus dem eigenen Objekt."]] : [["I read the floor plan", "With a background in building, I plan around the structure, not just the surfaces, and know when to bring in an engineer."], ["Specified for heavy use", "Over ten years planning healthcare and institutional buildings across Europe and Africa, where every surface has to be durable, easy to clean and still calm. That same standard goes into your rental."], ["And I let property myself", "I know handovers, turnovers and guest questions from running a property myself, not just from mood boards."]]).map(([h, p]) => <div key={h} className="border-t border-brand-border-primary py-6"><h3 className="font-heading text-2xl">{h}</h3><p className="mt-2 leading-relaxed text-brand-text-secondary">{p}</p></div>)}</div></div>
        <div className="mt-20 grid gap-10 border-t border-brand-border-primary pt-12 lg:grid-cols-2"><h2 className="font-heading text-4xl">{t.notForTitle}</h2><ul className="space-y-4 text-brand-text-secondary">{(isDe ? ["Eigentümer, die ausschließlich die billigste Einrichtung suchen.", "Wer eine Einkaufsliste um €200 erwartet.", "Hotelgruppen mit 40 oder mehr Einheiten.", "Wer den eigenen Geschmack reproduzieren möchte statt ein Produkt für einen definierten Mieter zu bauen."] : ["Owners looking only for the cheapest possible furnishing.", "Anyone expecting a €200 shopping list.", "Hotel groups with 40 or more units.", "Owners who want their personal taste reproduced rather than a product designed for a defined tenant."]).map((item) => <li key={item} className="flex gap-3"><Check className="mt-1 shrink-0" size={16} />{item}</li>)}</ul></div>
      </div></section>

      <section className="border-y border-brand-border-primary bg-brand-background-secondary px-5 py-20 md:px-10"><div className="mx-auto max-w-[900px]"><h2 className="text-balance font-heading text-4xl md:text-6xl">{t.faqTitle}</h2><div className="mt-10">{(isDe ? [
      ...
      ]).map(([q, a]) => <details key={q} className="group border-t border-brand-border-primary py-5"><summary className="flex cursor-pointer list-none items-center justify-between gap-5 font-heading text-2xl"><span>{q}</span><ChevronDown className="shrink-0 transition-transform group-open:rotate-180" size={20} /></summary><p className="max-w-2xl pt-4 leading-relaxed text-brand-text-secondary">{a}</p></details>)}</div></div></section>

      <ContactSection lang={lang} title={t.contactTitle} subtitle={t.contactSub} button={t.formButton} success={t.formSuccess} />
    </main>

    <footer className="border-t border-brand-border-primary px-5 py-8 md:px-10"><div className="mx-auto flex max-w-[1200px] flex-col gap-6 text-sm md:flex-row md:items-center md:justify-between"><div className="flex items-center gap-3"><strong className="font-heading text-xl">Jubili</strong><span className="text-brand-text-secondary">Vienna</span></div><nav className="flex flex-wrap gap-5"><a href="https://www.instagram.com/jubili.co" target="_blank" rel="noreferrer" aria-label="Jubili on Instagram">Instagram</a><a href="mailto:office@jubili.co">Email</a><a href="/impressum">Legal notice</a><a href="/datenschutz">Privacy policy</a></nav></div></footer>
  </div>;
}
...
function ContactSection({ lang, title, subtitle, button, success }: { lang: Lang; title: string; subtitle: string; button: string; success: string }) {
  ...
  return <section id="kontakt" className="scroll-mt-24 px-5 py-20 md:px-10 lg:py-28"><div className="mx-auto grid max-w-[1200px] gap-14 lg:grid-cols-[0.8fr_1.2fr]"><div><h2 className="text-balance font-heading text-4xl md:text-6xl">{title}</h2><p className="mt-5 text-lg leading-relaxed text-brand-text-secondary">{subtitle}</p><div className="mt-8"><Cta label="contact_call">{lang === "de" ? "Gespräch buchen" : "Book a call"}</Cta></div></div>
    ...
    ].map(([name, placeholder, type]) => <input key={name} name={name} type={type} required placeholder={placeholder} className="rounded-[4px] border border-brand-input-border bg-brand-input-background px-4 py-3.5 outline-none focus:ring-2 focus:ring-brand-accent-primary" />)}
      <select name="district" required defaultValue="" className="rounded-[4px] border border-brand-input-border bg-brand-input-background px-4 py-3.5"><option value="" disabled>{lang === "de" ? "Bezirk *" : "District *"}</option>{Array.from({ length: 23 }, (_, index) => <option key={index + 1}>{index + 1}{lang === "de" ? ". Bezirk" : ". District"}</option>)}</select>
      <select name="rentalType" defaultValue="" className="rounded-[4px] border border-brand-input-border bg-brand-input-background px-4 py-3.5"><option value="">{lang === "de" ? "Vermietungsart" : "Rental type"}</option><option value="short-term">{lang === "de" ? "Kurzzeit" : "Short-term"}</option><option value="mid-term-furnished">{lang === "de" ? "Mittelfristig möbliert" : "Furnished mid-term"}</option><option value="long-term">{lang === "de" ? "Unbefristet" : "Long-term"}</option><option value="unsure">{lang === "de" ? "Noch unklar" : "Not sure yet"}</option></select>
      <select name="budget" defaultValue="" className="rounded-[4px] border border-brand-input-border bg-brand-input-background px-4 py-3.5 sm:col-span-2"><option value="">{lang === "de" ? "Möbelbudget" : "Furniture budget"}</option><option value="under-10k">{lang === "de" ? "Unter €10k" : "Under €10k"}</option><option value="10-20k">€10–20k</option><option value="20-35k">€20–35k</option><option value="over-35k">{lang === "de" ? "Über €35k" : "Over €35k"}</option><option value="open">{lang === "de" ? "Noch offen" : "Not decided"}</option></select>
      <textarea name="message" rows={5} placeholder={lang === "de" ? "Worum geht es?" : "What is the project about?"} className="rounded-[4px] border border-brand-input-border bg-brand-input-background px-4 py-3.5 sm:col-span-2" />
      <button type="submit" disabled={status === "sending"} className="rounded-[4px] bg-brand-button-primary-background px-5 py-4 font-semibold text-brand-button-primary-text sm:col-span-2">{status === "sending" ? "…" : button}</button>
